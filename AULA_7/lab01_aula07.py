...
# Tema: Equilibrio entre varredura do espaco de busca (Exploration)
# e refino local (Exploitation).
# Codigo fornecido pronto no roteiro - executado sem alteracoes.
# ============================================================
import numpy as np
import matplotlib.pyplot as plt

np.random.seed(42)  # garante reprodutibilidade da escolha "aleatoria" das formigas

# Matriz de Distancias entre 5 Nos de Rede (Grafo)
...
beta = 2.0    # Influencia da Visibilidade/Heuristica
rho = 0.1     # Taxa de Evaporacao
...
def local_search_2opt(path, matrix):
    """
    Busca Local 2-opt.
    Tenta melhorar a rota realizando inversoes
...
print(
    f"[LAB 01 - SUCESSO] "
    f"Melhor Caminho: {[int(node) for node in best_path]} | "
    f"Custo: {best_cost}"
)
...
plt.grid()
plt.savefig("img_lab01_convergencia.png", dpi=110)
plt.show()
